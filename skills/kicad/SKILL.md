...
## On Matt's machines: the MS PCB Design Doctrine platform — read before any review

This skill is the generic analyzer. On Matt's machines it runs INSIDE the **MS PCB Design Doctrine platform** (rp1-home epic #1441). Always fold the points below into the review, because they are the house rules every board is measured against, not optional extras:

1. **Route first.** `~/.claude/PCB_DESIGN.md` maps every PCB / electronics ask to the skill, CLI tool and doctrine doc that already owns it. Read it once per session before planning.
2. **Doctrine rules.** `/home/matt/claudedocs/pcb-doctrine/DOCTRINE.md` is normative (`MSD-<DOMAIN>-<nn>` IDs; the S1–S6 schematic conventions are `MSD-SCH-10…60`); `doctrine-constitution.md` holds the 85 corpus-derived rules with provenance. Cite the MSD ID beside every finding it covers, and never restate a rule — reference it.
3. **Machine gate before opinions.** Run `python3 /home/matt/kicad-config/kicad-lint/cli.py <project-or-paths> --format json` first — five check families (S1–S6 style, KLC, 3D coverage, ERC/DRC ingestion, path-variable portability); exit 0 clean / 1 blocking / 2 tool error. Report its findings first; the manual review adds only what the gate cannot see.
4. **Render + LOOK.** After ANY edit to a `.kicad_sch` / `.kicad_pcb`, invoke `/kicad-visual-verify` (doctrine `MSD-PROC-50`) — ERC and DRC never catch visually-wrong-but-valid edits. Milestone reviews: `~/kicad-config/bin/kicad-3d-review <board.kicad_pcb>` (top/bottom/iso renders + GLB viewer, inline).
5. **Libraries and parts.** Symbols and footprints come from `ms-kicad-library` via `${MS_LIB}` (curated `ms-` tier vs quarantined `imported-` tier); part data from the atomic parts DB (`partsdb`, `.kicad_dbl`, Device Master Sheet sync). A placed part outside those is a doctrine deviation worth flagging. Protection / recovery primitives: MS-Blocks (`claudedocs/pcb-doctrine/ms-blocks-registry.md`).
6. **Fab.** Per-fab design rules and jobsets live in `/home/matt/kicad-config/dru/` and `/home/matt/kicad-config/jobsets/` (JLCPCB / PCBWay × 2 / 4-layer; IPC-2581 + ODB++ by default) — use them instead of hand-rolled export settings.
7. **Hard constraints.** KiCad 9.0.8 is the local ceiling and a KiCad 10 save one-way-gates a project; MS naming, never MG; every helper writes to a temp dir, never beside the source.
8. **Client deliverable?** For an SPD client review, `/spd-altium-intake` (Altium source) and `claudedocs/pcb-doctrine/spd-review-checklist.md` own the branded report format; `/jlcpcb-quote` owns the build quote.
